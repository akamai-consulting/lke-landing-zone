...
The `openbao-cert-watcher` Deployment (`instance-template/apl-values/components/openbao/openbao-cert-watcher.yaml`) detects this rotation and deletes the 3 OpenBao pods so they reload the new cert. The pods come back **sealed**.
