...
| `LINODE_DNS_TOKEN` | Consumed by Terraform (`TF_VAR_linode_dns_token`), not this workflow | Linode API token for apl-core's `cert-manager-webhook-linode` DNS-01 solver (`apps.cert-manager.dns.provider.linode.apiToken`) and ExternalDNS. No OpenBao seed is involved. ACME certificate challenges fail until it is provisioned. `bootstrap-dns.yml` only applies the `llz-letsencrypt-*` ClusterIssuers that target that webhook. |
