...
| `LLZAppScopeNotConverged` | An instance-owned Application (the operator escape hatch, or anything in the `instance-custom` AppProject) is hard-failed, or has been reconciling for an hour. It does NOT gate platform convergence by design — the platform contract excludes instance-owned content — so `llz_convergence_state` stays 0 and `LLZClusterNotConverged` is silent. The owner is the instance/app team, not the platform on-call. | Run `llz ci converge --scope=apps` for the per-resource verdict. Use **converge**, not `llz ci health`: outside a budget a pod that is merely being created classifies as failed, so the one-shot reports false hard failures on a healthy rollout. Note this alert is APPLICATION-level — a Synced+Healthy app whose ExternalSecrets/Jobs/Pods are broken does not move the gauge, and only the `--scope=apps` run (the blocking `app-scope-health` job in llz-scheduled-checks, weekly, or on demand) sees those. Commonest cause: a per-app credential the operator never seeded. |
...
| `LokiStatefulSetDegraded` | A Loki StatefulSet has **fewer ready replicas than desired** for 15m. Ingestion is degraded and the PDB blocks node drains (`disruptionsAllowed=0`), so cluster upgrades and node recycles hang. | `kubectl -n monitoring get sts -l app.kubernetes.io/name=loki` then `describe`/`logs <pod> --previous`. On an **ingester** the usual cause is an OOMKill during WAL replay, which **does not self-heal**: the WAL is replayed identically on every restart. `llz ci assert-loki` reports the running memory limit against the WAL-replay floor and whether the WAL is on a PVC. If the limit is below the floor, the apl-overlay is not reaching the chart — see [upstream-asks.md](../upstream-asks.md) §1. If `assert-loki` reports the WAL on an **emptyDir** on a cluster that predates the PVC change, that is the one-time StatefulSet migration, not a regression: `volumeClaimTemplates` is immutable, so enabling persistence cannot apply in place — the orphan-delete recipe is in the same section. Deleting the pod clears the loop at the cost of that replica's un-flushed chunks. |
| `LokiStatefulSetUnavailable` | A Loki StatefulSet has **0** ready replicas — log ingestion + queries are down. | As above, but total. Also check the S3 object-store credential path (`secret/obj/platform` → the `obj-secrets` Secret apl-core reads). |
