...
| `LLZ_AUTOMATION_TOKEN` | GitHub fine-grained PAT (Actions: read/write, Contents: write, Pull requests: write) for the two **opt-in** upstream-update workflows — `scheduled-apply.yml` dispatches `terraform.yml` with it, `template-upgrade.yml` pushes a branch and opens the upgrade PR. **Repo-level**. Empty unless you set `LLZ_SCHEDULED_APPLY=true` or `LLZ_TEMPLATE_UPGRADE=true`. It cannot be `GITHUB_TOKEN`: GitHub suppresses workflow runs from events raised with it, so the dispatch would create no run and the upgrade PR would run none of the gates that make it reviewable | **Manual**; ≤90-day policy, daily `token-inventory` expiry measurement when set. Unset → skipped entirely, not reported as `unknown`. **Its lapse is silent**: neither workflow runs more than monthly, so an expired token reads as "no upstream changes" rather than as a failure — which is why it is measured daily rather than trusted to fail loudly |
