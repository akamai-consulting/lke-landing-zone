...
| `cluster.bootstrap.aplChartVersion` | default | **Omit it** and the env tracks the llz release's apl-core baseline (`clusterspec.BaselineAplChartVersion`), so it moves forward with `copier update`. Set it only for a deliberate pin — an explicit value OVERRIDES the baseline and will NOT move when llz is upgraded. `llz validate` blocks a pin a full major behind the baseline (override: `LLZ_ALLOW_APL_CHART_MAJOR_DRIFT=1`) and warns on minor drift |
