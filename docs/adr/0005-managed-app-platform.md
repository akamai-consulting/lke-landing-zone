...
applies LLZ's class (same Linode-CSI provisioner, encryption + Retain) with the `is-default-class` annotation stripped
(`managedBlockStorageClassYAML`) — a NON-default class, so named PVCs bind without racing managed's own default.
Validated: SC applied non-default, managed default intact, a `block-storage-retain` PVC bound in ~10s.
