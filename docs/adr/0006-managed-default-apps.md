...
**Live-validation status:** the reachability field-unknown is RESOLVED (in-cluster-only → Job). Under e2e re-validation now: does the `alpine/git` Job pull + push from inside the cluster (github.com egress), does the repoint install harbor/loki/grafana/kyverno, and does kyverno's CRD land so `imageSignature` stops wedging. Remaining risks to watch: `alpine/git` image pullability on the managed cluster, and whether the operator's write-back races LLZ's force-push on the `apl-<env>` branch.
