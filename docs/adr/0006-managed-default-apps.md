...
1. **How is managed apl-core pointed at a github.com values branch?** The default `apl_enabled` cluster uses in-cluster gitea. Setting `otomi.git.repoUrl` to a github branch is itself a config write that (at provisioning) lives in gitea — a chicken-and-egg. Options to confirm: a provisioning-time hook, a one-time apl-api/config write, or a supported managed knob. **This is the load-bearing prerequisite for Path A and needs live confirmation against a managed cluster.** (If apl-core's values ARE on the github instance repo, note the bonus: apl-core's own ArgoCD then carries that repo credential, and the PR #306 `instanceRepoArgoSecretManifest` fix may become redundant — keep it as belt-and-suspenders.)
