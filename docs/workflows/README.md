...
| `llz-scheduled-apply.yml` | **Opt-in.** Applies every deployment on a cadence and blocks on the converge gate — the only route by which a merged Terraform change reaches a cluster without a human | [llz-scheduled-apply.md](llz-scheduled-apply.md) |
