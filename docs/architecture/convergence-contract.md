...
- **INSTANCE** (`instance-custom-<ns>` Applications) — the operator escape hatch generated from `kubernetes-custom/`. The convergence contract gates the **platform**; these Applications are instance-**owned** (`.template-manifest` `owned`, not `managed`), so a broken or still-settling one is **reported but excluded from the verdict** (exit `0`, like DEFERRED/DRIFT). A team's typo in a custom `ExternalSecret` (e.g. a stale `external-secrets.io/v1beta1`) must not fail the whole platform bootstrap. Gating states (`FAIL`/`PENDING`) are remapped to `INSTANCE` by `health.ClassifyArgoApp`; `OK` and cosmetic `DRIFT` are unchanged. The escape-hatch **mechanism** (directory-generator discovery + sync of a seeded manifest) is still hard-verified separately by `llz ci assert-instance-custom` in the release-e2e assert suite — that lane seeds a known-good manifest and is not subject to this tolerance.
