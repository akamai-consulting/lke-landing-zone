...
**Related:** [`docs/secrets.md`](../secrets.md) (architecture + dual-write), [`docs/runbooks/bootstrap-openbao.md`](../runbooks/bootstrap-openbao.md) (initial bootstrap), [`llz ci bao-configure`](https://github.com/akamai-consulting/lke-landing-zone/blob/main/tools/cmd/llz/ci_openbao_configure.go) (auth methods + policies definition).
...
- `bao auth enable <method>` — only ever needed during cluster bring-up; if you're enabling new auth methods on a live cluster, update [`llz ci bao-configure`](https://github.com/akamai-consulting/lke-landing-zone/blob/main/tools/cmd/llz/ci_openbao_configure.go) too so the next bootstrap reproduces the state.
...
1. **Write a policy** — enumerate the exact KV paths the new principal reads (no wildcards):
    ...
    Also add the policy + paths to [`llz ci bao-configure`](https://github.com/akamai-consulting/lke-landing-zone/blob/main/tools/cmd/llz/ci_openbao_configure.go) (so the next bootstrap reproduces it) and to the ExternalSecret-path validation used by the lint job (so it covers any new ExternalSecret refs).
...
After any deletion, remove the corresponding policy + binding from [`llz ci bao-configure`](https://github.com/akamai-consulting/lke-landing-zone/blob/main/tools/cmd/llz/ci_openbao_configure.go) so a future bootstrap doesn't re-create the principal.
