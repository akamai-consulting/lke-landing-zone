...
**Provider pins are not one of the things you have to move.** A release that raises
a provider constraint carries the matching `.terraform.lock.hcl` with it: the pin
and the `required_providers` constraint it satisfies are embedded together inside
`llz`, and `llz render` writes both into each root before every Terraform op. So
the upgrade that moves the constraint moves the pin, and neither is committed here.

This used to be the sharpest edge in the upgrade path, which is why it is called
out rather than left silent. The lock was `owned` — seeded when your instance was
scaffolded and never re-touched — while the constraint travelled the other way
inside the CI image. Raising one therefore stranded every existing instance below
the new floor, hard-blocked at the first step of every Terraform op, while a fresh
scaffold took both halves and passed every check. If you are upgrading across that
change, `llz upgrade` drops the stale lock from your index for you; commit the
removal with the rest of the upgrade.
