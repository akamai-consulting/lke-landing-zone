...
**Which means a release that raises a provider constraint leaves your lock behind,
and you have to move it.** The constraint travels inside the CI image; the pin is
`owned` and stays where it was scaffolded, so the two disagree from the moment you
upgrade. `llz upgrade` says so, and `llz ci provider-lock-guard --instance` fails
the upgrade PR until it is fixed — run it locally first:

```bash
(cd terraform-iac-bootstrap/<root> && llz tofu -- init -backend=false -upgrade -input=false)
git add terraform-iac-bootstrap/*/.terraform.lock.hcl && git commit
```

Both halves of that command are load-bearing.

`-backend=false` because your roots encrypt state at rest, so an init that
configures the backend needs `$TF_ENCRYPTION` and fails without it on OpenTofu's
own *"Invalid expression … A single static variable reference is required"*.
Regenerating a lock is a provider resolution and never touches state, so this form
needs no passphrase and no credentials at all.

**`llz tofu` because a bare `tofu` can fail silently here.** Your instance commits
the lock but no `.tf` — the roots are generated by `llz render` and gitignored — so
on a fresh clone those directories hold a lock and nothing else, and `tofu init`
there prints *"OpenTofu initialized in an empty directory!"*, **exits 0**, and
leaves the stale pin exactly where it was. `llz tofu` refuses that and tells you to
render; on an already-rendered tree it just runs.

Prefer not to pin at all? Delete the lock: with none committed, `tofu init`
resolves fresh inside the constraint and the guard passes.
