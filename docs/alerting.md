...
`tools/internal/extensions/assertobs/scrape.go` lists four ServiceMonitors (cert-manager,
...
2. If you add a new rule group, also add it to the `EXPECTED_RULES` list in the
   ...
   [tools/internal/extensions/assertobs/scrape.go](../tools/internal/extensions/assertobs/scrape.go) — the
