...
2. **Writes the inventory** — `llz ci token-inventory | kubectl apply -f -`
   ([`inventory.go`](../../tools/internal/extensions/lifecycle/tokeninv/inventory.go)) measures the
...
- `tools/internal/extensions/lifecycle/tokeninv/inventory.go` (+ `_test.go`) — add `--apply` (create-or-update via
  `kube.Client`).
