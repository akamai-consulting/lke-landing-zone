...
| [internal-extension-model](internal-extension-model.md) | Phases 1–2 — the declaration model (`tools/internal/extension`) and the first two extensions (`guard-budgets`, `guard-docs`, plus a registry and `llz extension list`). Nothing is dispatched through it yet: the declarations are inert and both gates still run as cobra commands. Action ABI, manifest, per-instance enablement and the remote half did not land; issue #399 sequences them. |
