...
- **Phase 0 (merged, #150).** The observe-only foundation:
  ...
  [`llz reconcile`](../../tools/internal/extensions/lifecycle/reconciler/reconcile.go) command (serves
...
- **Phase 2 (merged, #152).** The reconciler **manager**
  ([`reconcile_manager.go`](../../tools/internal/extensions/lifecycle/reconciler/reconcile_manager.go)) — runs N
...
- **Watch reconcilers (#153).** The manager gains an **event-triggered**
  loop ([`runWatchReconcilerLoop`](../../tools/internal/extensions/lifecycle/reconciler/reconcile_manager.go)): a
  ...
  ([`reconcile_argo_nudge.go`](../../tools/internal/extensions/lifecycle/reconcilelanes/argo_nudge.go)) —
...
- **Leader election (#154).** A minimal `coordination.k8s.io` Lease elector
  ([`reconcile_leader.go`](../../tools/internal/extensions/lifecycle/reconciler/reconcile_leader.go)) over the
...
- **Health gauges (this branch).** The observe reconciler also publishes the
  ...
  ([`reconcile_health.go`](../../tools/internal/extensions/lifecycle/reconciler/reconcile_health.go)), all over the
...
- **OpenBao-auth gauges (this branch).** An **opt-in, read-only** reconciler
  (`--reconcile-openbao-gauges`, [`reconcile_openbao.go`](../../tools/internal/extensions/lifecycle/reconcilelanes/openbao.go))
...
- **sc-demote (this branch).** The sixth CronJob conversion, and a **correction**:
  ...
  ([`reconcile_sc_demote.go`](../../tools/internal/extensions/lifecycle/reconcilelanes/sc_demote.go)) preserves
