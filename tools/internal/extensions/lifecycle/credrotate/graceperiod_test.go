...
// GRACE_DAYS=0 DRAINS THE PREDECESSOR IN THE SAME RUN, which is what the e2e
// assert lane sets it for. Callers capture `now` BEFORE they mint, so the
// replacement's `created` is a second or two later than the instant being judged
// against — and an unclamped supersededAt then beat even a zero-day cutoff, so
// each cycle leaked a live account:read_write PAT.
func TestAZeroGraceWindowDrainsTheCredentialJustSuperseded(t *testing.T) {
	now := int64(1_800_000_000)
	for _, mintSkew := range []int64{0, 1, 2, 30} {
		t.Run(fmt.Sprintf("minted %ds after now was read", mintSkew), func(t *testing.T) {
			sorted := []gracedToken{
				{ID: 2, Created: now + mintSkew}, // just minted, clock read earlier
				{ID: 1, Created: now - 60*day},   // the one it replaces
			}
			drain, inGrace := splitByGrace(sorted, 0, now)
			if len(drain) != 1 || drain[0] != 1 {
				t.Errorf("drain = %v (in grace %v), want [1]: at GRACE_DAYS=0 the predecessor "+
					"is superseded and has no overlap window to sit in", drain, inGrace)
			}
		})
	}
	// And a real window still protects it, so the clamp did not simply disable
	// the grace period.
	sorted := []gracedToken{{ID: 2, Created: now + 2}, {ID: 1, Created: now - 60*day}}
	if drain, _ := splitByGrace(sorted, 7, now); len(drain) != 0 {
		t.Errorf("drain = %v at GRACE_DAYS=7, want none", drain)
	}
}
