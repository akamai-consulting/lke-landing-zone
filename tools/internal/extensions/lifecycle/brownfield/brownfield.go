...
func RunScan(d Deps, o ScanOpts) error {
	...
	// Called unguarded, like every other Deps func field. `d.DryRun != nil && …`
	// fails OPEN: an unwired seam turns --dry-run silently off and this command
	// scans a live cluster having promised not to. A nil panic names the wiring
	// bug at the first invocation; the guard hides it behind correct-looking output.
	if d.DryRun() {
	...
	if !o.SkipCluster {
		ctx = o.Context
		if ctx == "" {
			if cur, err := kubectlCtx(d, o.Kubeconfig, "", "config", "current-context"); err == nil {
				ctx = strings.TrimSpace(cur)
			}
		}
		get = func(args ...string) string {
			out, err := kubectlCtx(d, o.Kubeconfig, o.Context, args...)
			if err != nil {
...
func kubectlCtx(d Deps, kubeconfig, context string, args ...string) (string, error) {
	var pre []string
	if kubeconfig != "" {
	...
	return d.KubectlOut(append(pre, args...)...)
}
...
// firstNonEmpty and orDash are local copies of package main's glue — three lines
// each, nothing to drift.
