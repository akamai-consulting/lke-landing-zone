...
// 6.1.0 release automation and has kept the convention since (v6.2.1 publishes as
// `version: v6.2.1`, tag `apl-v6.2.1`), and `helm --version 6.2.1` only resolves
// it via a fallback warning. Dropping the prefix on the next bump is a silent
// regression, so assert it here.
func TestDefaultAplChartVersion(t *testing.T) {
	if clusterspec.BaselineAplChartVersion != "v6.2.1" {
		t.Errorf("clusterspec.BaselineAplChartVersion = %q, want \"v6.2.1\" — bump deliberately, in lockstep with the platform baseline", clusterspec.BaselineAplChartVersion)
	}
