...
// llzGenerated reports whether these bytes are a promote.yml LLZ WROTE. Every
// renderer in this file opens with a `# GENERATED` banner and a "DO NOT EDIT BY
// HAND"; nothing an operator hand-writes does.
//
// IT IS THE DIFFERENCE BETWEEN MANAGING A FILE AND SEIZING ONE. The single-stage
// pipeline may only be written where there is nothing to destroy, which on a first
// run means an unrunnable file or one that applies nothing. That guard, alone, also
// locked llz out of the file on the SECOND run: the solo pipeline it had just
// written applies one deployment and names no undeclared one, so the next
// `llz env pipeline` fell through to the leave-alone arm and reported "llz is not
// managing this file" about a file llz had generated a second earlier — and, worse,
// stopped reporting drift on it, so a hand-edit to a stage read as "in sync".
// Recognising its own output closes both without touching the scar: an operator's
// hand-written promote.yml carries no banner and is still left exactly alone.
func llzGenerated(b []byte) bool {
	return bytes.HasPrefix(b, []byte("# GENERATED"))
}
...
func unmanagedNote(ranked, onDisk, declared int) string {
	// A ONE-DEPLOYMENT INSTANCE IS NOT WAITING ON A RANK. Reaching this arm with one
	// declared deployment means the operator maintains promote.yml by hand (llz's own
	// output is recognised and managed), and telling them to "set promotionRank on the
	// deployments you want chained" names a step that does not exist for them.
	if declared == 1 {
...
// guarantees len(stages) >= 1.
//
// ONE STAGE IS A SUPPORTED SHAPE, not a degenerate one. An instance with a single
// declared deployment has no ordering question to answer, and the chain machinery
// below already renders correctly for it — the entry stage chains from the
// preflight rather than from a prior stage, which is exactly what a lone stage
// wants. All that differs is the PROSE, because a file that calls itself a chain
// over ranks while listing one unranked deployment sends its reader looking for
// the rank they did not set. soloPipeline gates those sentences and nothing else.
func renderPromoteWorkflow(c promoCaller, stages []promoStage) string {
	solo := len(stages) == 1
	var b strings.Builder
	if solo {
		b.WriteString(`# GENERATED by ` + "`llz env pipeline`" + ` — the SINGLE-STAGE pipeline. This instance
# declares exactly one deployment, so there is no ordering question to answer and
# the pipeline is one stage that applies it. Dispatch this workflow and it runs;
# nothing has to be driven by hand.
#
# Declare a second deployment (` + "`llz env add <env>`" + `), set ` + "`promotionRank`" + ` on both and
# re-run ` + "`llz env pipeline`" + ` to grow this into a needs:-chained promotion.
# DO NOT EDIT BY HAND. See docs/environments-and-promotion.md.
`)
	}
	if !solo {
...
`)
	for i, s := range stages {
		...
		// A RANK THE SPEC NEVER SET MUST NOT BE PRINTED AS ONE. The solo pipeline's
		// deployment is normally unranked, and "(rank 0)" reads as a configured
		// value — it would send an operator looking for the 0 in their spec.
		if solo {
			b.WriteString(fmt.Sprintf("    name: Promote → %s (the only deployment)\n", s.name))
		} else {
...
func PlanWorkflow(d Deps, tfDir, relPrefix string) (Plan, error) {
	...
	if len(stages) < 2 {
		...
		// ── ONE DECLARED DEPLOYMENT IS NOT "NO PIPELINE" ────────────────────────
		//
		// It is a pipeline of length one, and the empty placeholder was the wrong
		// answer for it. An instance with a single deployment got a dispatchable
		// `Promote` that promoted NOTHING and a message telling the operator to go
		// run terraform.yml by hand — for the one topology where what to apply is
		// not in question. The rank exists to answer "which order", and with one
		// deployment there is no order to answer; requiring one made the feature
		// unreachable for the commonest starting shape.
		//
		// THE WRITE IS STILL GUARDED, because the scar above is real: it may only
		// happen when there is nothing to destroy. Two cases qualify — a file whose
		// stages name deployments that do not exist (unrunnable, the shape that
		// authorises every other write here) and a file that applies nothing at all
		// (the placeholder this replaces, or no file yet). A single-deployment
		// instance whose promote.yml ALREADY applies that deployment is left alone
		// like any other hand-maintained chain, however it is spelled.
		//
		// AN UNRESOLVABLE CALLER IS A SKIP, NOT AN ERROR. resolveCaller fails on a
		// tree with no rendered promote.yml or terraform.yml to copy the pin from and
		// no instance_repo in .copier-answers.yml. Returning that error made `--check`
		// on a bare checkout fail with "cannot determine the caller" — a complaint
		// about llz's ability to WRITE, raised by a mode that writes nothing, in place
		// of the verdict about the stages it was asked for. There is simply nothing to
		// render, so this falls through to the answers below that do not need one.
		soloWritable := len(missing) > 0 || len(promotingStages(onDisk)) == 0 || llzGenerated(got)
		if caller, cerr := resolveCaller(d, filepath.Dir(path)); cerr == nil &&
			len(declared) == 1 && soloWritable {
			// Rank 0 when the deployment carries none, which is the normal state
			// here — soloStage renders it as "the only deployment" rather than
			// printing a rank the spec never set.
			only := promoStage{name: declared[0]}
			if len(stages) == 1 {
				only = stages[0]
			}
			want := renderPromoteWorkflow(caller, []promoStage{only})
			if string(got) == want {
				return Plan{Path: path, Stages: onDisk, Undeclared: undeclared, Unresolved: unresolved, Declared: declared}, nil
			}
			return Plan{Path: path, Content: want, Changed: true, Order: []string{only.name},
				Stages: onDisk, Undeclared: undeclared, Unresolved: unresolved, Declared: declared,
				Note: fmt.Sprintf("promote.yml: %q is this instance's only deployment, so the pipeline is a single stage that applies it — dispatch Promote and it runs.", only.name)}, nil
		}

		if len(missing) == 0 {
			return Plan{Path: path, Stages: onDisk, Undeclared: undeclared, Unresolved: unresolved, Declared: declared,
				Note: unmanagedNote(len(stages), len(onDisk), len(declared))}, nil
		}
		// Undeclared stages AND too few ranks to regenerate a real chain from.
		// Abstaining here left the tree with no route back to green: `--check`
		// failed, and the fix it printed ("rank two deployments and regenerate") is
		// unreachable when you only have one. The empty pipeline is a state the
		// generator can render, so it renders it.
