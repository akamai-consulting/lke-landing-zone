...
// asMap decodes a JSON fixture the way the kube client hands it to the parsers.
...
func TestDesiredTagsFromSC(t *testing.T) {
	good := asMap(t, `{"parameters":{"linodebs.csi.linode.com/volumeTags":"block-storage, platform-support-services,lke123"}}`)
	...
	for _, bad := range []string{`{"parameters":{}}`, `{"parameters":{"linodebs.csi.linode.com/volumeTags":" , "}}`} {
		if _, err := desiredTagsFromSC(asMap(t, bad), "block-storage-retain"); err == nil {
