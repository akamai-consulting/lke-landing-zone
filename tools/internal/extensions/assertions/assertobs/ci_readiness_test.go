...
func TestRunCIWaitHarbor(t *testing.T) {
	...
	// Budget 0 so every case evaluates once: cigate.WaitPoll probes before it checks the
	...
	// carried no signal either way. Budget 0 so cigate.WaitPoll evaluates once instead of
