...
func RunAssertNoOrphans(d Deps, region, volumeRegion, clusterID, env string, threshold, attempts, retryDelay int) error {
	client, ctx, err := d.Client()
	if err != nil {
		return err
	}
...
func teardownLabels(d Deps, region, tfDir string) (tf.Labels, error) {
	if region == "" {
		return tf.Labels{}, fmt.Errorf("--region is required (the tfvars prefix, e.g. primary)")
	}
	vars, _, err := d.RegionTFVars(tfDir, region)
	if err != nil {
...
func tfOutputRaw(d Deps, dir, name string) string {
	out, err := d.Exec(d.TFBin(), "-chdir="+dir, "output", "-raw", name)
	if err != nil {
		return ""
	}
...
func RunCapture(d Deps, region, tfDir string) error {
	labels, err := teardownLabels(d, region, tfDir)
	if err != nil {
		return err
	}
	token, err := d.Token()
	...
	return d.Summary("GITHUB_ENV",
		"LKE_CLUSTER_ID="+clusterID,
		"CLUSTER_PVC_VOLUME_IDS="+volIDs)
}

func RunForceDelete(d Deps, region, tfDir string) error {
	labels, err := teardownLabels(d, region, tfDir)
	if err != nil {
		return err
	}
	...
	fwID := numericOrEmpty(tfOutputRaw(d, tfDir, "node_firewall_id"))
	if fwID == "" {
		label := firstNonEmpty(tfOutputRaw(d, tfDir, "node_firewall_label"), labels.Firewall)
...
func RunDeleteVPC(d Deps, region, tfDir, clusterID string, attempts, retryDelay int, requireDeleted bool) error {
	labels, err := teardownLabels(d, region, tfDir)
	if err != nil {
		return err
	}
	...
	vpcID := numericOrEmpty(tfOutputRaw(d, tfDir, "vpc_id"))
	if vpcID == "" {
		vpcs, err := client.ListVPCs(ctx)
		if err != nil {
...
// firstNonEmpty returns the first non-empty string. A local copy of package main's
// helper: three-line glue, not a shared rule — nothing can drift.
...
// own. Copied from package main's ci.go rather than shared: it is two calls into
// internal/linode, and the linode package is the authority both sides read.
