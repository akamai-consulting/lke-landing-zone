...
// FIELD-by-field, not whole-toggle — so a per-env sizing tweak inherits the
// default's enabled state, and an explicit enable/disable inherits default
// sizing). Returns nil when neither side sets anything, so the built-in
...
func mergeComponents(base, over map[string]ComponentToggle) map[string]ComponentToggle {
	...
	for k, v := range over {
		out[k] = mergeToggle(out[k], v) // out[k] is the zero toggle when k is env-only
	}
	return out
}
...
func pickIntPtr(base, over *int) *int {
	if over != nil {
		return over
	}
	return base
