...
// landingzone.yaml is rejected (LoadSplit) so clusters/ stays the single source.
...
// an Environment (cluster + recipes): clusters/<env>.yaml carries one cluster
...
// plus every clusters/*.yaml (one ClusterDefinition each). Each env inherits
...
func LoadSplit(root string) (*LandingZone, error) {
	...
	// Deployments are authored as clusters/<env>.yaml, never inline — keep
	// landingzone.yaml to identity + shared defaults so clusters/ is the one place
	// an env is defined.
	if len(lz.Spec.Environments) > 0 {
		return nil, fmt.Errorf("%s must not define spec.environments — author each deployment as %s/<env>.yaml", LandingZoneFile, ClustersDir)
	}
