...
func TestLintReportsRepoRelativePathsFromASubdirectory(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(outside, "untracked.go"), []byte("package x\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	want := []string{
		"tools/sub/tracked.go", "tools/sub/untracked.go", "other/untracked.go",
	}
		...
		for _, w := range want {
			if !contains(got, w) {
				t.Errorf("run from %q: changed set = %v, want repo-relative %q — "+
					"lint's routing regexes are anchored and skip anything else", from, got, w)
			}
		}
	}
}

// RELATIVITY IS A DEFAULT, NOT A FACT. `diff.relative=true` makes `git diff
// --name-only` CWD-relative as well, and from a subdirectory it then prints
// NOTHING for a file outside it — the empty set again, out of the half that was
// supposed to be the reliable one. The other tests here cannot see it: they run
// with GIT_CONFIG_GLOBAL=/dev/null, which is exactly what hides a global setting.
func TestLintIsUnaffectedByDiffRelative(t *testing.T) {
	dir := newRepo(t)
	sub := filepath.Join(dir, "tools", "sub")
	if err := os.MkdirAll(sub, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(sub, "keep.go"), []byte("package x\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	git(t, dir, "add", "-A")
	git(t, dir, "commit", "-qm", "sub")
	// A TRACKED edit outside the subdirectory: the half diff.relative governs.
	if err := os.WriteFile(filepath.Join(dir, "tracked.go"), []byte("package x // edited\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	git(t, dir, "config", "diff.relative", "true")

	if got := lintChanged(t, sub); !contains(got, "tracked.go") {
		t.Errorf("with diff.relative=true, run from a subdirectory: changed set = %v, "+
			"want tracked.go — a modified file outside the CWD must not vanish", got)
	}
}
