...
var allowedEdges = map[string]bool{
	...
	"lint -> render":                     true,
	"lint -> sustain":                    true,
	"lint -> templatemanifest":           true,
	"manifestguard -> brownfield":        true,
	"mtlsguard -> openbao":               true,
	"newinstance -> envdef":              true,
	"newinstance -> instanceresolve":     true,
	"newinstance -> onboard":             true,
	"objenc -> baoseed":                  true,
	"onboard -> assertplatform":          true,
	"onboard -> branchpolicy":            true,
	"onboard -> configreadiness":         true,
	"onboard -> credrotate":              true,
	"onboard -> doctor":                  true,
	"onboard -> envtopology":             true,
	"onboard -> instanceresolve":         true,
	"onboard -> reachability":            true,
	"onboard -> statepassphrase":         true,
	"onboard -> templatecommit":          true,
	"openbao -> assertobs":               true,
	"openbao -> baolifecycle":            true,
	"openbao -> baoseed":                 true,
	"openbao -> credrotate":              true,
	"openbao -> envtopology":             true,
	"reachability -> baolifecycle":       true,
	"reachability -> converge":           true,
	"reachability -> instanceresolve":    true,
	"reconcilelanes -> openbao":          true,
	"reconciler -> credrotate":           true,
	"reconciler -> openbao":              true,
	"reconciler -> reconcilelanes":       true,
	"reconciler -> tokeninv":             true,
	"reconciler -> volumes":              true,
	"releasepublish -> chartguard":       true,
	"render -> buildpreflight":           true,
	"render -> envdef":                   true,
	"render -> instanceresolve":          true,
	"seedspecial -> identityconfig":      true,
	"selfupgrade -> templatemanifest":    true,
	"statepassphrase -> buildpreflight":  true,
	"teardown -> credrotate":             true,
	"teardown -> instanceresolve":        true,
	"templatecommit -> pincoherence":     true,
	"templatecommit -> sustain":          true,
	"templatecommit -> versionpins":      true,
	"tokeninv -> reconcilelanes":         true,
	"upgrade -> buildpreflight":          true,
	"upgrade -> onboard":                 true,
	"upgrade -> render":                  true,
	"upgrade -> selfupgrade":             true,
	"upgrade -> sustain":                 true,
	"upgrade -> templatecommit":          true,
	"upgrade -> templatemanifest":        true,
}
