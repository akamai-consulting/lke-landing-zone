...
func TestSomeExtensionNamesDifferFromTheirPackage(t *testing.T) {
	...
	// sixty-five"` finds them.
	const documented = 33 // registry.go (Package), package_test.go (above), cli/extension.go (listVerbose)
	...
	if total := len(All()); total != 65 {
		t.Errorf("the registry holds %d extensions; the same three comments say sixty-five. "+
