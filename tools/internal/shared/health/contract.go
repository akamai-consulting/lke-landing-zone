...
type Report struct {
	...
	// Instance holds findings for instance-OWNED Applications — the operator
	// escape hatch generated by the instance-custom ApplicationSet from
	// kubernetes-custom/ (instance-custom-<ns>). The convergence contract gates the
	// PLATFORM; these are the operator's own manifests (.template-manifest `owned`,
	// not `managed`), so a broken or still-settling one is REPORTED but must never
	// pin the gate — that would let a team's typo fail the whole platform bootstrap.
	// The escape hatch itself is proven separately by `llz ci assert-instance-custom`
	// in release-e2e. Excluded from Verdict() exactly like Deferred/Drift. See
	// health.IsInstanceCustomApp and ClassifyArgoApp.
	Instance []string
...
// AddInstance records an instance-owned escape-hatch finding (an instance-custom
// Application that is failed or still settling). Reported for the operator, but —
// like Deferred/Drift — excluded from the verdict, so instance-owned content never
// gates the platform convergence contract.
func (r *Report) AddInstance(msg string) { r.Instance = append(r.Instance, msg) }
