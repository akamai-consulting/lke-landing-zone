...
// GITEA MUST STILL BE TURNED OFF, and nothing pinned that until this test. On
// managed, the per-env overlay was the ONLY channel that disabled apl-core's
// in-cluster gitea — instance-template/apl-values/values.yaml is not rendered
// there — so gating the per-env layer on EmitOnManaged silently removed it, and
// a new cluster would have kept gitea running with the unencrypted gitea-valkey
// PVC that health/workloads.go documents.
//
// It belongs in the _shared list rather than the per-env one: LLZ has a stated
// reason (apl-core 6.x gates apl-gitea-operator on gitea.enabled, and with
// BYO-Git that clone path is dead weight), which is what that list is for, as
// against an app whose toggle LLZ merely happens to know about.
func TestGiteaIsStillDisabledSomewhereInTheOverlay(t *testing.T) {
	var doc appsOverlayDoc
	if err := yaml.Unmarshal([]byte(RenderAppsOverlayShared()), &doc); err != nil {
		t.Fatalf("unmarshal shared apps overlay: %v", err)
	}
	tog, ok := doc.Apps["gitea"]
	if !ok {
		t.Fatal("gitea is absent from the _shared apps overlay, and the per-env layer no longer " +
			"names it either — nothing disables apl-core's in-cluster gitea on managed, and " +
			"instance-template/apl-values/values.yaml is not rendered there")
	}
	if tog.Enabled {
		t.Errorf("gitea: enabled=%v, want false", tog.Enabled)
	}
	// And NOT from the per-env layer, or the two would fight over one key.
	if _, ok := appsOverlay(t, defaultBoot(), allComponentsOn())["gitea"]; ok {
		t.Error("gitea is emitted by BOTH layers; the _shared one is the policy statement")
	}
}

// THE _shared LIST STAYS IN LOCKSTEP WITH THE VALUES BLOCK IT MIRRORS. Its own
// comment says so, and gitea was missing from it while values.yaml carried it —
// which is how removing the per-env emission left nothing behind.
func TestTheStaticDisabledListCoversEveryAppLLZKeepsOff(t *testing.T) {
	var doc appsOverlayDoc
	if err := yaml.Unmarshal([]byte(RenderAppsOverlayShared()), &doc); err != nil {
		t.Fatal(err)
	}
	for _, app := range []string{"gitea", "knative", "kserve", "kubeflow-pipelines", "linode-cfw", "rabbitmq", "tekton"} {
		tog, ok := doc.Apps[app]
		if !ok {
			t.Errorf("%q is missing from the _shared apps overlay", app)
			continue
		}
		if tog.Enabled {
			t.Errorf("%q: enabled=true in a list of apps LLZ keeps off", app)
		}
	}
}
