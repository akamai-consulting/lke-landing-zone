...
var citedCounts = map[extension.Grant]struct {
	...
	extension.ReadRepo:      {62, 48, "repo.go: \"declared by 48 of 68 extensions — more than any other grant\"; model doc distribution table"},
...
func TestHandleHeaderCensusesMatchTheRegistry(t *testing.T) {
	...
	if total != 68 {
		t.Errorf("the registry holds %d extensions; repo.go's header says 68. Update the "+
