...
var citedCounts = map[extension.Grant]struct {
	...
	extension.ReadRepo:      {65, 51, "repo.go: \"declared by 51 of 71 extensions — more than any other grant\"; model doc distribution table"},
...
func TestHandleHeaderCensusesMatchTheRegistry(t *testing.T) {
	...
	if total != 71 {
		t.Errorf("the registry holds %d extensions; repo.go's header says 71. Update the "+
