...
var citedCounts = map[extension.Grant]struct {
	...
	extension.ReadRepo:      {57, 44, "repo.go: \"declared by 44 of 64 extensions — more than any other grant\"; model doc distribution table"},
...
func TestHandleHeaderCensusesMatchTheRegistry(t *testing.T) {
	...
	if total != 64 {
		t.Errorf("the registry holds %d extensions; repo.go's header says 64. Update the "+
