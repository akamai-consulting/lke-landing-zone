...
var citedCounts = map[extension.Grant]struct {
	...
	extension.ReadRepo:      {58, 45, "repo.go: \"declared by 45 of 65 extensions — more than any other grant\"; model doc distribution table"},
...
func TestHandleHeaderCensusesMatchTheRegistry(t *testing.T) {
	...
	if total != 65 {
		t.Errorf("the registry holds %d extensions; repo.go's header says 65. Update the "+
