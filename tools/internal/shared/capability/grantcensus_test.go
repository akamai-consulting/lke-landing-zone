...
var citedCounts = map[extension.Grant]struct {
	...
	extension.ReadRepo:      {61, 47, "repo.go: \"declared by 47 of 67 extensions — more than any other grant\"; model doc distribution table"},
...
func TestHandleHeaderCensusesMatchTheRegistry(t *testing.T) {
	...
	if total != 67 {
		t.Errorf("the registry holds %d extensions; repo.go's header says 67. Update the "+
