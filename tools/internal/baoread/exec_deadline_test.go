...
// WaitForState spends its budget by ACCUMULATING the interval, not by reading a
// clock:
//
...
//
// The cap is the same safeguard pollRecorder's clock jump provides: WaitForState
...
func baoStatusScript(t *testing.T, reads map[string]int, unsealAt, maxReads int) {
	...
	// DumpDiagnostics shells out for the stuck pod's container log; keep it
...
func TestWaitForAutoUnsealPollsEveryFiveSeconds(t *testing.T) {
	...
	wantEverySleepAt(t, *slept, 5*time.Second, 2*len(PodNames))
}
...
func TestWaitForAutoUnsealLeaderTimeoutIsBudgetOverInterval(t *testing.T) {
	...
	if want := "leader " + PodNames[0] + " not initialized+unsealed within 20s"; err.Error() != want {
	...
	// overrun the budget, so the wait gives up. DumpDiagnostics adds one read.
	if reads[PodNames[0]] != 6 {
		t.Fatalf("leader read %d times, want 6 (5 polls over a 20s budget at 5s, plus the timeout diagnostic)", reads[PodNames[0]])
	}
