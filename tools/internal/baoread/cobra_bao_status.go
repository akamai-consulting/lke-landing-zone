...
// cobra_bao_status.go — the `llz ci bao-status` flag set, and the GHA step-output
...
func BaoStatusCmd() *cobra.Command {
...
func RunCIBaoStatus() error {
	states := make([]PodStatus, 0, len(PodNames))
	for _, pod := range PodNames {
		...
		st, _ := ParsePodStatus(out)
		fmt.Printf("  %s: initialized=%t sealed=%t\n", pod, st.Initialized, st.Sealed)
		states = append(states, st)
	}
	initialized, sealedAny := AggregateStatus(states)
