...
// ci_releasepublish.go — the `llz ci publish-charts` and `pin-instance-images`
// flag sets.
//
...
func ciPublishChartsCmd() *cobra.Command {
	var o releasepublish.PublishChartsOpts
	var interval int
	c := &cobra.Command{
		Use:   "publish-charts",
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return releasepublish.RunPublishCharts(o)
		},
	}
...
func ciPinInstanceImagesCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			return releasepublish.RunPinInstanceImages(releasepublish.PinImagesOpts{
				...
				Retries:        timeout / releasepublish.Max1(interval),
