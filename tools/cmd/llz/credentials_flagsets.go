...
// credentials_flagsets.go — the `llz credentials pat` and `llz credentials
// lke-admin` flag sets.
//
// The rotators are tools/internal/credrotate, which also owns the shared
// framework they run on. What stays here is the cobra wiring and the help text.
...
func credentialsPATCmd(o *credrotate.Opts) *cobra.Command {
	c := &cobra.Command{
		Use:   "pat",
...
func credentialsLKEAdminCmd(o *credrotate.Opts) *cobra.Command {
	c := &cobra.Command{
		Use:   "lke-admin",
	...
	rotate := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			return credrotate.RunLKEAdminRotate(o, firstNonEmpty(clusterID, os.Getenv("LKE_CLUSTER_ID")))
		},
	}
...
func credentialsPATCreateCmd(o *credrotate.Opts) *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return credrotate.RunPATCreate(context.Background(), credrotate.NewPATClient(token), apply, label, scopes, validityDays, ghaSecretName, strings.Fields(ghaDeployments))
		},
	}
	f := c.Flags()
...
func credentialsPATRevokeOldCmd(o *credrotate.Opts) *cobra.Command {
	var label string
	var graceDays int64
	c := &cobra.Command{
		Use:   "revoke-old",
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return credrotate.RunPATRevokeOld(context.Background(), credrotate.NewPATClient(token), apply, label, graceDays)
		},
	}
	f := c.Flags()
...
func credentialsObjKeyCmd(o *credrotate.Opts) *cobra.Command {
	c := &cobra.Command{
		Use:   "obj-key",
...
func credentialsObjKeyCreateCmd(o *credrotate.Opts) *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return credrotate.RunObjKeyCreate(context.Background(), credrotate.NewObjKeyClient(token), apply, label, cluster, bucket, permissions, ghaAccessName, ghaSecretName, strings.Fields(ghaDeployments))
		},
	}
...
func credentialsObjKeyRevokeOldCmd(o *credrotate.Opts) *cobra.Command {
	var label string
	...
	c := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return credrotate.RunObjKeyRevokeOld(context.Background(), credrotate.NewObjKeyClient(token), apply, label, keepNewest)
		},
	}
	f := c.Flags()
