...
var ghSecretTargets = []struct {
	...
	{"TF_STATE_ACCESS_KEY", "on-demand"},
	{"TF_STATE_SECRET_KEY", "on-demand"},
	...
	{"TF_STATE_ENCRYPTION_PASSPHRASE", "on-demand"},
}

// NOTE — the class strings above are literals, not the credClass* constants,
// because those constants live in reconcile_openbao.go and arrive with the
// credential-coverage PR. Until that merges, `on-demand` matches NO alert rule
// (main's LLZCredentialRotationOverdue is scoped to class="automated"), so these
// series are published but inert: visible on the dashboard, not yet alerting.
// TestGHSecretTargetClassesAreKnown pins the vocabulary so the two cannot drift.
