...
func reportOrphanedStateKeys(ctx context.Context, c objKeyLister, instanceRepo string) {
	...
	fmt.Printf("\n%s %d existing object-storage key(s) already carry %q.\n", yellow("!"), len(ids), label)
	fmt.Println(dim("      A key's secret half is shown once, so an existing one cannot be reused —"))
	fmt.Println(dim("      this run mints a fresh key and the old ones are superseded by the push."))
	fmt.Printf("      %s %v\n", dim("ids:"), ids)
	fmt.Println(dim("      Once the push has landed and a Terraform run has succeeded, revoke them in"))
	fmt.Println(dim("      the Linode console (Object Storage → Access Keys). Not automated: the repo"))
	fmt.Println(dim("      still points at one of them until the push below succeeds."))
}
