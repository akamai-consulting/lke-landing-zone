...
// ci_argodiag.go — the `llz ci diagnose-argocd` flag set.
//
// The dump itself is tools/internal/argodiag, which declares the extension. What
// stays here is the cobra wiring and the help text.
//
...
func ciDiagnoseArgoCDCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error { return argodiag.Run(aplNS, ns) },
	}
