...
func TestWaitForAplOverlayPreconditionAlreadyMetReturnsWithoutKicking(t *testing.T) {
	...
	if kicked {
		t.Error("kicked on an already-satisfied precondition — a redundant pass on every pod start")
	}
}
...
// satisfied would hot-loop the lane at the reconnect cadence for the pod's life.
func TestWatchAplOverlayPreconditionHoldsUntilContextDone(t *testing.T) {
	setAplOverlayEnv(t)
	withFastPreconditionPoll(t)
	seeded := withObjCredSeams(t, nil)
	seeded.Store(true) // satisfied up front → the waiter returns at once
