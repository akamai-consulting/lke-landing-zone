...
func checkBucketLabelsAvailable(prefix, env string) {
	...
	fmt.Printf("  %s %d of this deployment's bucket labels already exist on YOUR account (%s).\n",
		yellow("!"), len(mine), dim(mine[0]+", …"))
	fmt.Println(dim("      Expected on a re-apply — Terraform adopts them. If an apply instead fails with"))
	fmt.Println(dim("      `[400] … already exists`, the label is taken on ANOTHER account: Linode bucket"))
	fmt.Println(dim("      labels share one namespace per region globally."))
	fmt.Printf("      %s\n", dim("Fix that by changing the PREFIX, not the deployment name:"))
	fmt.Printf("      %s\n", cyan("llz spec set instance.objLabelPrefix=<something-unique>"))
}
