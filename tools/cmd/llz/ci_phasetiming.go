...
// ci_phasetiming.go — the four `phase-timing` flag sets.
//
...
func ciPhaseMarkCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			return phasetiming.AppendPhaseMark(phasetiming.PhaseLogPath(logPath), args[0], phasetiming.NowMilli())
		},
	}
...
func ciPhaseReportCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			return phasetiming.RunPhaseReport(phasetiming.PhaseLogPath(logPath), out, title)
		},
	}
	c.Flags().StringVar(&logPath, "log", "", "marks log path (default $LLZ_PHASE_LOG or a temp file)")
...
func ciCollectTimingCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			return phasetiming.RunCollectTiming(dir, title, imagePulls, aplOperator)
		},
	}
...
func ciCollectImagePullsCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Args: cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error { return phasetiming.RunCollectImagePulls(out) },
	}
