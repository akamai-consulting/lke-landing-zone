...
func runRender(g globalOpts, env string, tfvarsOnly, check bool) error {
	...
	for _, name := range envs {
		...
		if !tfvarsOnly {
			if err := renderManifest(name, e.Components, aplDir, relPrefix, dryRun); err != nil {
...
func committedTargets(env string, components map[string]clusterspec.ComponentToggle, aplDir string) (map[string]string, error) {
	...
	// apl-core backend: patch apps.<key>.enabled into the shared values.yaml
	// template. Skipped (not an error) for instances without the example overlay.
	if base, err := os.ReadFile(filepath.Join(aplDir, "example", "values.yaml")); err == nil {
		rendered, err := clusterspec.RenderValues(base, components)
		if err != nil {
...
func renderManifest(env string, components map[string]clusterspec.ComponentToggle, aplDir, relPrefix string, dryRun bool) error {
	targets, err := committedTargets(env, components, aplDir)
	if err != nil {
		return err
	}
...
func checkManifestDrift(lz *clusterspec.LandingZone, aplDir string, envs []string) error {
	...
	for _, name := range envs {
		e, _ := lz.Env(name)
		targets, err := committedTargets(name, e.Components, aplDir)
		if err != nil {
			return err
		}
