...
func reapVolumes(ctx context.Context, client volumeReapClient, o reapOpts, del func(path, desc string)) error {
	...
	now := time.Now()
	matched := false
	// skipped counts Volumes the SCOPE filter excluded (wrong label prefix, region,
	// id, tag); the kept* counters below count Volumes the scope ADMITTED and the
	// cluster-liveness gate then spared. Separate on purpose — the first means "not
	// in the set I was asked about", the second "in the set, and deliberately kept".
	skipped := 0
	keptLive, keptYoung, keptUntagged := 0, 0, 0
	for _, v := range vols {
