...
func e2eRequirements(admin bool) []requirement {
	reqs := []requirement{
		...
		// REPO-level, not infra-<env>: the `vpc` root's state is shared across the
		// deployments that attach to one network (vpc/<network>/terraform.tfstate),
		// so a per-env passphrase would let one env encrypt a state file its peer
		// cannot decrypt. One key per instance is also one thing to escrow.
		{"TF_STATE_ENCRYPTION_PASSPHRASE", true, false, true, false, "OpenTofu state+plan encryption key — GENERATED by `llz tokens`; ESCROW IT OFFLINE"},
...
// environment rather than at repo level. Derived from the requirements table so
// the scope is declared once, next to the credential, instead of being inferred
// from the name at each push site.
//
// Unknown names default to env-scoped: that has been the behaviour for every
// secret llz pushes, and the failure mode is the milder one (visible to fewer
// jobs) rather than the leakier one.
