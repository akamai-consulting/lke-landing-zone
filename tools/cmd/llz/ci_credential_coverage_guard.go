...
// LITERAL LIST in Go — ghPATTargets, ghSecretTargets, credPaths. A list is only
...
// to credPaths and its own gate (`assert-rotation-health`), and is stated here so
...
var credCoverageExempt = map[string]struct {
	...
	"LINODE_API_TOKEN": {
		...
		reason: "the broad account PAT. Its expiry IS measured — gatherLinodeTokens lists every " +
			...
			"its ROTATION AGE rides secret/linode/broad-pat in credPaths, class automated. " +
