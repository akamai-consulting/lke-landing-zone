...
- [`instance-template/`](instance-template/) — genericized starter material an
  ...
  (`cluster`, `databases`, `object-storage`, `vpc` — generated by `llz render`, not committed), the
  `apl-values` base (`values.yaml`; the heavy `platform-apl/` manifests stay in the
  template repo root and are pulled as pinned kustomize remote refs, not vendored),
  from which `llz render` generates each env's thin overlay, and instance GitHub
  workflows + composite actions. The
...
instead of the `curl` line. After the build, escrow the OpenBao seal/recovery keys
— see the quickstart's post-build steps.
