...
- `loki_admin_password` (sensitive) — Loki gateway basic-auth admin password.
  Always supplied via `TF_VAR_loki_admin_password`: the `llz-terraform` workflow
  runs `llz ci ensure-env-secret` before this apply, which generates+stashes the
  infra-`<env>` `LOKI_ADMIN_PASSWORD` secret on first run and reuses it thereafter
  (no longer generated by Terraform or held in TF state).
