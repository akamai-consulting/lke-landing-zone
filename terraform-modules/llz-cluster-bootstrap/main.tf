resource "local_file" "apl_rendered_values" {
  ...
  content         = var.apl_rendered_values
  file_permission = "0600"

  lifecycle {
    precondition {
      condition     = var.env_revision_in_configmap == var.apps_repo_revision
      error_message = <<-EOM
        env-revision-configmap.yaml mismatch detected.

          apl-values/${var.apl_values_env}/manifest/env-revision-configmap.yaml
          data.revision = "${var.env_revision_in_configmap}"
        ...
        Fix:
          1. Either set APPS_REPO_REVISION on infra-${var.apl_values_env}
             to "${var.env_revision_in_configmap}", OR
...
resource "helm_release" "apl" {
  ...
  values = [var.apl_rendered_values]
...
# NOTE (apl-core 6.x): the former `apl_values_repo_creds` resource (Secret
# `apl-values-repo` in ns argocd, secret-type=repository, seeded with
# var.apl_values_repo_url + the PAT) was removed here — its N4-M1 open question
# is resolved: on v6, apl-core self-registers the values-repo credential at
# helmfile time (values/argocd/argocd-raw.gotmpl renders an `argocd-repo-creds-*`
